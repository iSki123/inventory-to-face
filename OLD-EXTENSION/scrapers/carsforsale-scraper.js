// CarsForSale.com simple scraper content script
(function(){
  ...
  function scrape(){
    const cards = Array.from(document.querySelectorAll('[data-qa="vehicle-card"], .vehicle-card, .srp-grid .vehicle, .vehicle-item, article'));
    const vehicles=[];
    ...
    if(vehicles.length){
      chrome.runtime.sendMessage({ action: 'scrapedInventory', source: 'carsforsale', vehicles });
    }
  }
